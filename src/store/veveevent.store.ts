...
export const usePoiStore = create<PoiStore>((set) => ({
  ...
  pois: [] as VeveEvent[],
  //----------------------------------------------------------------------------------------------------------------------------------------------------
  selectedPoi: null as VeveEvent | null,
  //----------------------------------------------------------------------------------------------------------------------------------------------------
  error: null,
  //----------------------------------------------------------------------------------------------------------------------------------------------------
  addPoi: (poi: VeveEvent) =>
    set((state) => ({ ...state, pois: [...state.pois, poi], error: null })),
  //----------------------------------------------------------------------------------------------------------------------------------------------------
  removePoi: (id: number) =>
    set((state) => ({
      ...state,
  ...
  setSelectedPoi: (poi: VeveEvent | null) =>
    set((state) => ({ ...state, selectedPoi: poi })),
}));

// ------------------------------------------------------------------------------------------------------------------------ Hooks personnalisés ajustés
export const usePoiStoreSelectors = () => {
  const { pois, selectedPoi, error } = usePoiStore((state) => ({
    pois: state.pois,
  ...
  const selectPoiByLocation = (lat: number, lng: number) =>
    pois.find((poi) => poi.lat === lat && poi.lng === lng);
  const selectPoiByName = (name: string) =>
    pois.find((poi) => poi.name === name);
